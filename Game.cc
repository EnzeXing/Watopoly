...
Game::Game(std::map<std::string, std::string> p) {
    ...
    buildings.emplace_back(std::make_shared<NeedlesHall>("Needles Hall", 7, rimcup));
    ...
    buildings.emplace_back(std::make_shared<NeedlesHall>("Needles Hall", 22, rimcup));
    ...
    buildings.emplace_back(std::make_shared<NeedlesHall>("Needles Hall", 36, rimcup));
...
Game::Game(std::ifstream & file) {
    ...
    for (int i = 0; i < 40; ++i) {
        ...
        if (owner != "BANK") {
            ss1 >> improvement;
            auto academic = std::dynamic_pointer_cast<Academic>(buildings[i]);
...
void Game::saveGame(std::ofstream & file) {
    ...
    for (auto n : buildings) {
        file << n->getName() << " ";
        auto n2 = std::dynamic_pointer_cast<Academic>(n);
